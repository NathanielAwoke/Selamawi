<?php 
	...
	if ( valid_form($_POST, $set_name)) {
		...
		if(strlen($fname)===0 or strlen($mname)===0 or strlen($lname)==0)
		{
			storeFeedback(MSG_SETTING,"Empty fields are not allowed",MSG_ERR);
	...
	//change password
		...
		if(strlen(trim($_POST['set_npassword']))==0 or strlen(trim($_POST['set_cpassword']))==0){
			storeFeedback(MSG_SETTING,"You cant leave fields empty",MSG_ERR);
			redirect(PAGE_SETTING);
		}
