<?php 
	...
	class Account{
		...
		//sorts based on SENAYT algorithm
		public function popularPetitions()
		{

			$query = "SELECT *, (num_comments * " . FACT_COM . " + num_up_votes*" . FACT_UP ." - num_down_votes*" . FACT_DOWN . ") as score FROM " . TABLE_PET . " WHERE status<> " . PET_ST_DEL ." and sent=" . PET_UNRESOLVED  . " ORDER BY score DESC LIMIT " . self::TOP;
		...
		public function myInterest($tags) 
		{
			...
			$query = "SELECT * FROM " . TABLE_PET ." WHERE status<> " . PET_ST_DEL ." and petID IN (SELECT petID from " . TABLE_PET_TAG ." WHERE tagID IN $formated and audience IN $audiences group by petID) and sent=" . PET_UNRESOLVED ;
